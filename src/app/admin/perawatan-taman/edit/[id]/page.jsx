// src/app/admin/perawatan-taman/edit/[id]/page.jsx
"use client";
...
export default function EditGardenCarePackagePage() {
  ...
  useEffect(() => {
    const fetchPackageData = async () => {
      ...
      try {
        const response = await fetch(`/api/gardencares/${id}`);
        const packageToEdit = await response.json();

        if (response.ok) {
          let packageWithThreeOptions = { ...packageToEdit };
          if (packageWithThreeOptions.options.length < 3) {
            ...
            for (
              let i = packageWithThreeOptions.options.length;
              i < 3;
              i++
            ) {
              packageWithThreeOptions.options.push(defaultOptions[i]);
            }
          }
          if (packageWithThreeOptions.options.length > 3) {
            packageWithThreeOptions.options = packageWithThreeOptions.options.slice(
              0,
              3
            );
          }

          setPackageData(packageWithThreeOptions);
        } else {
          setNotFound(true);
        }
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        alert("Paket perawatan berhasil diperbarui!");
        router.push("/admin/perawatan-taman");
      } else {
        const data = await response.json();
        alert(`Gagal memperbarui paket: ${data.error}`);
      }
    } catch (error) {
      ...
      <div className="p-1 bg-white">
        <div className="mb-6">
          ...
          <p className="text-gray-600 mt-1">
            Paket perawatan dengan ID tersebut tidak ditemukan.
          </p>
          <button
            onClick={() => router.push("/admin/perawatan-taman")}
            className="mt-4 flex items-center px-4 py-2 bg-[#50806B] text-white rounded-lg hover:bg-[#3d6854] transition-colors duration-300"
          >
    ...
    <div className="p-1 bg-white">
      <div className="mb-6">
        <div className="flex items-center mb-2">
          ...
          <h1 className="text-3xl font-bold text-gray-800">Edit Paket Perawatan</h1>
        </div>
        <p className="text-gray-600">
          Perbarui paket perawatan di inventaris Green Garden
        </p>
      </div>
