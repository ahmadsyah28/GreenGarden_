...
export default function EditDesainPage({ params }) {
  ...
  // Fetch design and categories data
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        if (desainResponse.ok && categoriesResponse.ok) {
          ...
          // Set form data with design details
          setFormData({
            name: desainData.name || "",
            category: desainData.category || "",
            ...
            suitableFor: desainData.suitableFor || "",
            mainPlants: desainData.mainPlants?.length > 0 ? desainData.mainPlants : [""],
            estimatedTimeToFinish: desainData.estimatedTimeToFinish || "",
            additionalServices: desainData.additionalServices?.length > 0 
              ? desainData.additionalServices.map(service => ({
                  name: service.name,
                  price: new Intl.NumberFormat("id-ID").format(service.price)
                })) 
              : [{ name: "", price: "" }]
          });
          ...
          if (desainData.image) {
            setImagePreview(desainData.image);
            setOriginalImage(desainData.image);
          }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Send data to API
      const response = await fetch(`/api/desains/${id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (response.ok) {
        ...
        alert(`Gagal memperbarui desain: ${data.error}`);
      }
    } catch (error) {
      console.error('Error:', error);
      alert('Terjadi kesalahan saat memperbarui desain.');
    } finally {
      setIsSubmitting(false);
    }
  };
    ...
    <div className="p-1">
      <div className="mb-6">
        <div className="flex items-center mb-2">
          ...
          <h1 className="text-3xl font-bold text-gray-800">Edit Desain</h1>
        </div>
        <p className="text-gray-600">Edit desain taman di inventaris Green Garden</p>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-md">
        <form onSubmit={handleSubmit} className="p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {/* Left Column */}
            <div>
              <h2 className="text-xl font-semibold text-gray-800 mb-4">Informasi Desain</h2>
              ...
              {/* Suitable For */}
              <div className="mb-4">
                <label htmlFor="suitableFor" className="block text-sm font-medium text-gray-700 mb-1">
                  Cocok Untuk
                </label>
                <input
                  type="text"
              ...
              {/* Estimated Time */}
              <div className="mb-4">
                <label htmlFor="estimatedTimeToFinish" className="block text-sm font-medium text-gray-700 mb-1">
                ...
                {formData.features.map((feature, index) => (
                  <div key={`feature-${index}`} className="flex mb-2">
                    ...
                    <button
                      ...
                      className={`ml-2 p-2 rounded-lg ${formData.features.length === 1 ? 'bg-gray-200 text-gray-400 cursor-not-allowed' : 'bg-red-100 text-red-600 hover:bg-red-200'}`}
                    >
                      &times;
                    </button>
                  </div>
                ...
                {formData.mainPlants.map((plant, index) => (
                  <div key={`plant-${index}`} className="flex mb-2">
                    <input
                      type="text"
                      value={plant}
                      onChange={(e) => handleArrayChange(index, 'mainPlants', e.target.value)}
                      className="flex-grow px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#50806B] focus:border-transparent"
                    ...
                    <button
                      ...
                      className={`ml-2 p-2 rounded-lg ${formData.mainPlants.length === 1 ? 'bg-gray-200 text-gray-400 cursor-not-allowed' : 'bg-red-100 text-red-600 hover:bg-red-200'}`}
                    >
                      &times;
                    </button>
                  </div>
                  ...
                  <div key={`service-${index}`} className="flex flex-col mb-3">
                    <div className="flex mb-1">
                      ...
                      <button
                        ...
                        className={`ml-2 p-2 rounded-lg ${formData.additionalServices.length === 1 ? 'bg-gray-200 text-gray-400 cursor-not-allowed' : 'bg-red-100 text-red-600 hover:bg-red-200'}`}
                      >
                        &times;
                      </button>
                    </div>
